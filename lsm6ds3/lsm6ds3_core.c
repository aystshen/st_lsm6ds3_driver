...
#define LSM6DS3_GYRO_FS_245_GAIN		1 //8750
#define LSM6DS3_GYRO_FS_500_GAIN		875 //17500
#define LSM6DS3_GYRO_FS_1000_GAIN		1750 //35000
#define LSM6DS3_GYRO_FS_2000_GAIN		7000 //70000
...
static const struct lsm6ds3_sensor_name {
	...
	[LSM6DS3_ACCEL] = {
		.name = "accel",
		.description = "gsensor", // ST LSM6DS3 Accelerometer Sensor
	},
	[LSM6DS3_GYRO] = {
		.name = "gyro",
		.description = "gyro", // ST LSM6DS3 Gyroscope Sensor
	},
	[LSM6DS3_SIGN_MOTION] = {
		.name = "sign_m",
							...
							const char *description)
{
	...
	__set_bit(INPUT_EVENT_TIME_MSB, sdata->input_dev->relbit);
	__set_bit(INPUT_EVENT_TIME_LSB, sdata->input_dev->relbit);
	__set_bit(INPUT_EVENT_MISC, sdata->input_dev->relbit);

	if ((sdata->sindex == LSM6DS3_ACCEL) ||
					(sdata->sindex == LSM6DS3_GYRO)) {
		__set_bit(INPUT_EVENT_X, sdata->input_dev->relbit);
		__set_bit(INPUT_EVENT_Y, sdata->input_dev->relbit);
		__set_bit(INPUT_EVENT_Z, sdata->input_dev->relbit);
	}
							...
							s32 data, int64_t timestamp)
{
	...
	input_event(input, INPUT_EVENT_TYPE, INPUT_EVENT_MISC, data);
	input_event(input, INPUT_EVENT_TYPE, INPUT_EVENT_TIME_MSB,
							timestamp >> 32);
	input_event(input, INPUT_EVENT_TYPE, INPUT_EVENT_TIME_LSB,
							timestamp & 0xffffffff);
...
int lsm6ds3_allocate_workqueue(struct lsm6ds3_data *cdata)
{
    ...
    if (gpio_is_valid(cdata->irq_gpio) || cdata->irq > 0) {
		if (gpio_is_valid(cdata->irq_gpio))
		...
		if (err < 0) {
 			dev_err(cdata->dev, "failed to request irq %d\n", cdata->irq);
			return err;
		}
	} else {
        dev_err(cdata->dev, "failed to request irq, gpio invalid\n");
        return -EINVAL;
	}
...
static u32 lsm6ds3_parse_dt(struct lsm6ds3_data *cdata)
{
    ...
    cdata->irq_gpio = of_get_named_gpio(np, "st,irq_gpio", 0);
	if (!gpio_is_valid(cdata->irq_gpio)) {
		dev_err(cdata->dev, "No valid irq gpio");
        return -ENODEV;
	}
...
static int lsm6ds3_dev_open(struct inode *inode, struct file *filp)
{
	int ret = 0;
    
	dev_info(g_cdata->dev,
		 "device node major=%d, minor=%d\n", imajor(inode), iminor(inode));

	return ret;
}

static long lsm6ds3_dev_ioctl(struct file *pfile,
					 unsigned int cmd, unsigned long arg)
{
    ...
    struct inode *inode;
	struct lsm6ds3_sensor_data *sdata;
    struct lsm6ds3_data *cdata = g_cdata;

    inode = pfile->f_path.dentry->d_inode;
    minor = MINOR(inode->i_rdev);
    if (minor < 0 || minor >= LSM6DS3_SENSORS_NUMB) {
        dev_err(cdata->dev, "%s, minor invalid\n", __func__);
        return -EFAULT;
    }

    if (_IOC_TYPE(cmd) != LSM6DS3_IOC_MAGIC) 
        return -EINVAL;
    if (_IOC_NR(cmd) > LSM6DS3_IOC_MAXNR) 
        return -EINVAL;
    ...
    if (copy_from_user(&data, (int *)arg, sizeof(int))) {
        dev_err(cdata->dev, "%s, copy from user failed\n", __func__);
        return -EFAULT;
    }
    
    dev_info(cdata->dev, "%s, minor=%d, cmd=%d, data=%d\n", __func__, minor, cmd, data);

    sdata = &cdata->sensors[minor];
	switch (cmd) {
    	case LSM6DS3_IOC_ENABLE:
            if (data > 0) {
...
static const struct file_operations lsm6ds3_dev_fops = {
	.owner = THIS_MODULE,
    .open = lsm6ds3_dev_open,
	.unlocked_ioctl = lsm6ds3_dev_ioctl
};

static struct miscdevice lsm6ds3_devices[LSM6DS3_SENSORS_NUMB] = {
	[LSM6DS3_ACCEL] = {
		.minor = LSM6DS3_ACCEL,
	    .name = "accel",
	    .fops = &lsm6ds3_dev_fops,
	},
	[LSM6DS3_GYRO] = {
		.minor = LSM6DS3_GYRO,
	    .name = "gyro",
	...
	[LSM6DS3_SIGN_MOTION] = {
		.minor = LSM6DS3_SIGN_MOTION,
	    .name = "sign_m",
	    .fops = &lsm6ds3_dev_fops,
	},
	[LSM6DS3_STEP_COUNTER] = {
		.minor = LSM6DS3_STEP_COUNTER,
	    .name = "step_c",
	    .fops = &lsm6ds3_dev_fops,
	},
	[LSM6DS3_STEP_DETECTOR] = {
		.minor = LSM6DS3_STEP_DETECTOR,
	    .name = "step_d",
	    .fops = &lsm6ds3_dev_fops,
	},
	[LSM6DS3_TILT] = {
...
static int lsm6ds3_create_dev(struct lsm6ds3_data *cdata) {
    int err, i;

    for (i = 0; i < LSM6DS3_SENSORS_NUMB; i++) {
        err = misc_register(&lsm6ds3_devices[i]);
        if (err < 0) {
            return err;
        }
    }
...
int lsm6ds3_common_probe(struct lsm6ds3_data *cdata, u16 bustype)
{
	...
	if (err < 0) {
        dev_err(cdata->dev, "failed to parse dt\n");
        return err;
	}
#else /* CONFIG_OF */
	...
	if (err < 0) {
        dev_err(cdata->dev, "failed to init sensors\n");
    ...
    err = lsm6ds3_create_dev(cdata);
    if (err < 0) {
        dev_err(cdata->dev, "failed to create device\n");
		goto exit_free_io_port;
	}
